...
if ($alerts.StatusCode -ne 200) {
    Write-Information "##vso[task.logissue type=error] Error getting alerts from Azure DevOps Advanced Security:", $alerts.StatusCode, $alerts.StatusDescription
    exit 1
}
...
if ($alertsTemp.StatusCode -ne 200) {
    Write-Information "##vso[task.logissue type=error] Error getting alerts from Azure DevOps Advanced Security:", $alertsTemp.StatusCode, $alertsTemp.StatusDescription
    exit 1
}
...
if ($failingAlerts.Count -gt 0) {
    ...
    Write-Information $errorText
    $failingAlerts | Format-Table -AutoSize | Out-String | Write-Information
    exit 1
}
else {
    Write-Information "##vso[task.complete result=Succeeded;]DONE"
    exit 0
}